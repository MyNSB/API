...
func DeleteHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// Get student details from sessions and determine if they are an admin
	user, err := Parse.ParseSessions(r, w)
	if err != nil || !User.IsAdmin(user) || !Util.ExistsString(user.Permissions, "sportsDep") {
		Util.Error(403, "Forbidden", "student not logged in or does not have sufficient privileges", "Error!", w)
		return
	}
